...
def news():
    ...
    lis = soup.find_all("li", {"class": "live-li"})
    for li in lis:

        timetxt = li.find("time").text.strip()
        timetxt = cTime(timetxt)
        content = li.find("span", {"class": "detail"}).text.strip()
        content = content.replace("查看图片","").strip()
        passage = u'\U0001F3E0' + " %s (%s)" % (content, timetxt)
        passageid = hashlib.md5(str.encode(content[:20])).hexdigest()
